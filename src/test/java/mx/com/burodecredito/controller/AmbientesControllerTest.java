...
@MicronautTest
@Slf4j
class AmbientesControllerTest {

    private static final String API_URI = "/ambientes";
    ...
    void testUpdate() {
        ...
        Ambientes response = client.toBlocking().retrieve(request, Ambientes.class);
        log.info("Respuesta {}", response);
        assertEquals(idPrueba, response.getIdAmbiente());
    ...
    private void addData() {
        ...
        log.info("Respuesta {}", response);
        assertNotNull(response);
        idPrueba = response.getIdAmbiente();
    }
}