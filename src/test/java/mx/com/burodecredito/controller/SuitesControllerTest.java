...
@MicronautTest
@Slf4j
class SuitesControllerTest {

    private static final String API_URI = "/suites";
    ...
    @Order(4)
    void testUpdate() {
        Suites params = new Suites();
        params.setIdSuite(idPrueba);
        params.setNombre("suite actualizada");
        params.setDescripcion("prueba");



        HttpRequest<Object> request = HttpRequest.PUT(API_URI, params);
        Suites response = client.toBlocking().retrieve(request, Suites.class);
        log.info("Respuesta {}", response);
        assertEquals(idPrueba, response.getIdSuite());
